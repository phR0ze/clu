...
impl Clu {
    ...
    pub fn with_config_dir<T: AsRef<Path>>(mut self, path: Option<T>) -> CluResult<Self> {
        if let Some(x) = path {
    ...
    pub fn with_data_dir<T: AsRef<Path>>(mut self, path: Option<T>) -> CluResult<Self> {
        if let Some(x) = path {
    ...
    pub fn init(&mut self) -> CluResult<()> {
        if self.init {
            return Ok(());
        }
    ...
    pub fn save_config(&self) -> CluResult<()> {
    ...
    pub fn info<T: AsRef<str>>(&mut self, pkgs: &[T]) -> CluResult<()> {
    ...
    pub fn remove<T: AsRef<[Component]>>(&mut self, components: T) -> CluResult<()> {
    ...
    pub(crate) fn load_config(&mut self) -> CluResult<()> {
    ...
    // Implement support for write*! macro varients to use Clu as a Writer.
...
//     // Test setup
//     fn setup<T: AsRef<Path>>(path: T) -> Clu {
